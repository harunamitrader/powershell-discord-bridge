...
詳しい使い方と skill 設定方法は `docs\advanced-local-ai-slot-send.md` を見てください。Copilot 用の skill テンプレートは `docs\skill-examples\multicli-discord-bridge-slot-send\SKILL.md` に置いてあります。

連携時だけ読む軽量 shared state として、次も使えます。

```powershell
npm run slot:observe -- --state
```

これにより `%APPDATA%\multicli-discord-bridge\coordination\slot-state.json` を読めます。内容は各 slot の `cwd`、`status`、`updatedAt`、`foregroundCommand`、`recentInbound` です。`foregroundCommand` は **最後の `promptReady` 後に最初に Enter 付きで送られたコマンド**だけを記録します。

この state 確認を自然文から使いたい場合は、Copilot 用の別テンプレート `docs\skill-examples\multicli-discord-bridge-slot-state\SKILL.md` も使えます。
