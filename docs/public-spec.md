...
- Discord とは別に、**advanced 向けのローカル自動化機能**として、ローカル専用 automation endpoint から `slot + from + text + optional Enter` の最小リクエストを送れます。送信前には対象 slot をアプリ側でアクティブ化し、送信時には `[from: ...]` ヘッダーを自動付与します
...
- ユーザーが求めたときだけ、local automation から slot coordination state JSON、slot visible text、slot screenshot (`!ss` 相当)、app window screenshot (`!wss` 相当) を取得できます
...
- ローカル automation endpoint は Electron アプリ起動中のみ利用でき、`npm run slot:send -- --slot slot3 --from human --text "..."` または `node .\scripts\bridge-send-slot.cjs --slot slot3 --from human` で使えます。`--from` は必須で、送信時は `[from: ...]` ヘッダーを自動付与します。skill 設定例は `docs\advanced-local-ai-slot-send.md` と `docs\skill-examples\multicli-discord-bridge-slot-send\SKILL.md` を参照してください
