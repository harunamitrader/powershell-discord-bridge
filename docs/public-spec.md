...
- 同じチャンネルに送ったメッセージは、同じ slot のセッションへ順番に送られます
...
- soft timeout / hard timeout / fixed bridge dimensions は Electron アプリ側設定で変更できます
- 各 slot の既定作業ディレクトリと全体既定ディレクトリは Electron アプリ側設定で変更できます
